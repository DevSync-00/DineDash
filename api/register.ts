...
export default async function handler(req: any, res: any) {
  ...
  try {
    // Safely extract request body parsing strings if necessary
    let body = req.body;
    ...
    // Direct Supabase setup inside Vercel Function
    ...
    if (url && anonKey) {
      try {
        ...
        if (error) {
          console.error("❌ [Vercel-Supabase] Insert failed:", error.message);
          supabaseError = error.message;
        } else {
          console.log(`✅ [Vercel-Supabase] Successfully saved registration for "${payload.restaurantName}"`);
          savedToSupabase = true;
        }
      } catch (err: any) {
        console.error("❌ [Vercel-Supabase] Unexpected error:", err);
        supabaseError = err.message;
      }
    } else {
      console.log(`💡 [Vercel-Supabase] Skipped save (Mock Mode) for "${payload.restaurantName}".`);
      savedToSupabase = true; // For simulation/testing with clean status
    }
